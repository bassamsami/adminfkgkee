...
async function updateLinksWithNewUrl(newUrl) {
    try {
        ...
        if (stMatch && expMatch && dataMatch && hmacMatch) {
            ...
            alert("الرابط الجديد لا يحتوي على جميع القيم المطلوبة (st, exp, data, hmac).");
        }
    } catch (error) {
...
async function updateCollectionLinks(collectionName, newSt, newExp, newData, newHmac, linkType) {
    try {
        ...
        snapshot.forEach(async (doc) => {
            ...
            // تحديث الروابط بناءً على النوع
            if (oldUrl && oldUrl.includes(linkType)) {
...
document.getElementById("save-new-url-btn").addEventListener("click", () => {
    ...
    if (newUrl) {
        ...
        updateLinksWithNewUrl(newUrl); // تحديث يدوي
    } else {
